...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        # ETHZ ASL: Apply same resizing to depth_prior
        if 'depth_prior' in sample.keys():
            sample['depth_prior'] = cv2.resize(sample["depth_prior"], (width, height), interpolation=cv2.INTER_NEAREST)
        # ETHZ ASL: End
...
class Crop(object):
    ...
    def __call__(self, sample):
        ...
        # ETHZ ASL: Crop depth prior as well, same as img
        if 'depth_prior' in sample.keys():
            sample['depth_prior'] = sample['depth_prior'][h_start: h_end, w_start: w_end]
        # ETHZ ASL: End
